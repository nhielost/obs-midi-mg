...
class MMGActionTimeout : public MMGAction {
  public:
  explicit MMGActionTimeout() { blog(LOG_DEBUG, "Empty action created."); };
  explicit MMGActionTimeout(const QJsonObject &json_obj);
  enum Actions { TIMEOUT_MS, TIMEOUT_S };
  ...
  Category get_category() const override { return Category::MMGACTION_TIMEOUT; }

  MMGUtils::MMGNumber &num1() override { return time; };
  const MMGUtils::MMGNumber &num1() const override { return time; };
