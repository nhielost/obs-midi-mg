...
class MMGAction {
  ...
  enum class ReplayBuffer { REPBUF_ON, REPBUF_OFF, REPBUF_TOGGLE_ONOFF, REPBUF_SAVE };
  ...
  {
    if (index >= 0 && index < 4)
      return (nums_state & (3 << (index << 1))) >> (index << 1);
    throw 1;
  };
  ...
  {
    if (index >= 0 && index < 4)
      nums_state = (nums_state & ~(3 << (index << 1))) | (value << (index << 1));
  };
  ...
  do_obs_source_enum(QComboBox *list,
		     MMGAction::Category restriction_t = MMGAction::Category::MMGACTION_NONE,
		     const QString &rest = "");
  static void
  ...
  static void do_mmg_binding_enum(QComboBox *list, const QString &current_binding,
  ...
  template<typename T>
  static void do_action_specific(const MMGAction *params, const MMGMessage *midi);
};
