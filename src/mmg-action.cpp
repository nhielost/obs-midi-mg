...
double num_or_value(const MMGAction *params, const MMGMessage *midi, int num,
		    double default_value = 0, double mult = 128.0, double value_oper = 0.0)
{
  if (params->get_num_state(num) == 1) {
    return qRound(((midi->get_value() + value_oper) / 128.0) * mult);
...
void MMGAction::do_obs_source_enum(QComboBox *list, MMGAction::Category restriction_t,
				   const QString &rest)
{
  R r;
  r.c = list;
  ...
  obs_enum_all_sources(
	...
	if (!OBSSceneItemAutoRelease(obs_scene_sceneitem_from_source(
	      OBSSceneAutoRelease(obs_get_scene_by_name(r->r_v.qtocs())), source)))
	  return true;
	break;
...
void MMGAction::do_obs_media_enum(QComboBox *list, MMGAction::Category restriction_t,
				  const QString &rest)
{
  R r;
  r.c = list;
...
void MMGAction::do_obs_transition_enum(QComboBox *list, MMGAction::Category restriction_t,
				       const QString &rest)
{
  R r;
  r.c = list;
...
void MMGAction::do_obs_filter_enum(QComboBox *list, MMGAction::Category restriction_t,
				   const QString &rest)
{
  if (restriction_t == MMGAction::Category::MMGACTION_SOURCE_VIDEO) {
    OBSSourceAutoRelease source = obs_get_source_by_name(rest.qtocs());
...
void MMGAction::do_obs_hotkey_enum(QComboBox *list)
{
  obs_enum_hotkeys(
    [](void *param, obs_hotkey_id id, obs_hotkey_t *hotkey) {
      ...
      // From obs-midi
      if (name.contains("libobs") || name.contains("MediaSource") || name.contains("OBSBasic"))
	return true;
...
void MMGAction::do_mmg_binding_enum(QComboBox *list, const QString &current_binding,
...
template<>
void MMGAction::do_action_specific<MMGAction::None>(const MMGAction *params, const MMGMessage *midi)
{
  Q_UNUSED(midi);
...
template<>
void MMGAction::do_action_specific<MMGAction::VirtualCam>(const MMGAction *params,
							  const MMGMessage *midi)
{
  Q_UNUSED(midi);
...
template<>
void MMGAction::do_action_specific<MMGAction::ReplayBuffer>(const MMGAction *params,
							    const MMGMessage *midi)
{
  ...
  if ((QString(config_get_string(obs_config, "Output", "Mode")) == "Simple" &&
    ...
    params->blog(LOG_INFO, "<Replay Buffer> action failed - Replay Buffers are not enabled.");
    return;
  }
...
template<>
void MMGAction::do_action_specific<MMGAction::StudioMode>(const MMGAction *params,
							  const MMGMessage *midi)
{
  if (params->get_str(0) == "Use Message Value" &&
      (uint)midi->get_value() >= get_obs_scene_count()) {
    params->blog(LOG_INFO, "<Studio Mode> action failed - MIDI value exceeded scene count.");
    return;
  }
  ...
  OBSSourceAutoRelease source_obs_scene = obs_get_source_by_name(
    params->get_str(0) == "Use Message Value" ? scene_names[midi->get_value()]
						      ...
						      const MMGMessage *midi)
{
  if (params->get_str(0) == "Use Message Value" &&
      (uint)midi->get_value() >= get_obs_scene_count()) {
    params->blog(LOG_INFO, "<Scene Switching> action failed - MIDI value exceeded scene count.");
    return;
  }
  char **scene_names = obs_frontend_get_scene_names();
  OBSSourceAutoRelease source_obs_scene = obs_get_source_by_name(
  ...
  case MMGAction::Scenes::SCENE_SCENE:
    if (!source_obs_scene) {
      params->blog(LOG_INFO, "<Scene Switching> action failed - Scene does not exist.");
      bfree(scene_names);
      return;
    }
...
template<>
void MMGAction::do_action_specific<MMGAction::VideoSources>(const MMGAction *params,
							    const MMGMessage *midi)
{
  OBSSourceAutoRelease obs_scene_source = obs_get_source_by_name(params->get_str(0).qtocs());
  OBSSourceAutoRelease obs_source = obs_get_source_by_name(params->get_str(1).qtocs());
  if (!obs_source || !obs_scene_source) {
    params->blog(LOG_INFO, "<Video Sources> action failed - Scene or source does not exist.");
    return;
  }
  OBSSceneItemAutoRelease obs_sceneitem =
    obs_scene_sceneitem_from_source(obs_scene_from_source(obs_scene_source), obs_source);
  if (!obs_sceneitem) {
    params->blog(LOG_INFO, "<Video Sources> action failed - Source does not exist in scene.");
    return;
  }
  vec2 coordinates;
  ...
  case MMGAction::VideoSources::SOURCE_VIDEO_POSITION:
    ...
    vec2_set(&coordinates, num_or_value(params, midi, 0, coordinates.x, get_obs_dimensions().first),
	     num_or_value(params, midi, 1, coordinates.y, get_obs_dimensions().second));
  ...
  case MMGAction::VideoSources::SOURCE_VIDEO_DISPLAY:
    if (params->get_str(2) == "Toggle") {
      ...
      obs_sceneitem_set_visible(obs_sceneitem, bool_from_str(params->get_str(2)));
    }
    break;
  case MMGAction::VideoSources::SOURCE_VIDEO_LOCKED:
    if (params->get_str(2) == "Toggle") {
      ...
      obs_sceneitem_set_locked(obs_sceneitem, bool_from_str(params->get_str(2)));
    }
    break;
  case MMGAction::VideoSources::SOURCE_VIDEO_CROP:
    ...
    crop.bottom = num_or_value(params, midi, 2, crop.bottom, length);
  ...
  case MMGAction::VideoSources::SOURCE_VIDEO_ALIGNMENT:
    if (params->get_num_state(0) == 1 && midi->get_value() > 8) {
      params->blog(LOG_INFO, "<Video Sources> action failed - MIDI value exceeded choice options.");
      return;
    }
  ...
  case MMGAction::VideoSources::SOURCE_VIDEO_SCALE:
    ...
    vec2_set(&coordinates, num_or_value(params, midi, 0, coordinates.x * length, 100, 1) / length,
	     num_or_value(params, midi, 1, coordinates.y * length, 100, 1) / length);
  ...
  case MMGAction::VideoSources::SOURCE_VIDEO_SCALEFILTER:
    ...
    obs_sceneitem_set_scale_filter(obs_sceneitem, (obs_scale_type)num_or_value(params, midi, 0));
    break;
  case MMGAction::VideoSources::SOURCE_VIDEO_ROTATION:
    obs_sceneitem_set_rot(obs_sceneitem, num_or_value(params, midi, 0, 0, 360.0));
    break;
  case MMGAction::VideoSources::SOURCE_VIDEO_BOUNDING_BOX_TYPE:
    ...
    obs_sceneitem_set_bounds_type(obs_sceneitem, (obs_bounds_type)num_or_value(params, midi, 0));
    break;
  ...
  case MMGAction::VideoSources::SOURCE_VIDEO_BLEND_MODE:
    ...
    obs_sceneitem_set_blending_mode(obs_sceneitem,
...
template<>
void MMGAction::do_action_specific<MMGAction::AudioSources>(const MMGAction *params,
							    const MMGMessage *midi)
{
  OBSSourceAutoRelease obs_source = obs_get_source_by_name(params->get_str(0).qtocs());
  if (!obs_source) {
    params->blog(LOG_INFO, "<Audio Sources> action failed - Source does not exist.");
    return;
  }
  if (!(obs_source_get_output_flags(obs_source) & OBS_SOURCE_AUDIO)) {
    params->blog(LOG_INFO, "<Audio Sources> action failed - Source is not an audio source.");
    return;
  }
  ...
  case MMGAction::AudioSources::SOURCE_AUDIO_VOLUME_CHANGETO:
    ...
    if (params->get_num_state(0) == 1 && midi->get_value() == 0) {
      ...
      obs_source_set_volume(obs_source,
			    std::pow((num_or_value(params, midi, 0, 0, 100.0, 1) / 100.0), 3.0));
    }
    break;
  case MMGAction::AudioSources::SOURCE_AUDIO_VOLUME_CHANGEBY:
    if (std::cbrt(obs_source_get_volume(obs_source)) * 100.0 + params->get_num(0) >= 100.0) {
      obs_source_set_volume(obs_source, 1);
    } else if (std::cbrt(obs_source_get_volume(obs_source)) * 100.0 + params->get_num(0) <= 0.0) {
      obs_source_set_volume(obs_source, 0);
    } else {
      obs_source_set_volume(
	obs_source,
	std::pow(std::cbrt(obs_source_get_volume(obs_source)) + (params->get_num(0) / 100.0), 3.0));
    }
    break;
  ...
  case MMGAction::AudioSources::SOURCE_AUDIO_OFFSET:
    ...
    obs_source_set_sync_offset(obs_source, (num_or_value(params, midi, 0, 0, 3200.0) * 1000000));
    break;
  case MMGAction::AudioSources::SOURCE_AUDIO_MONITOR:
    if (params->get_num_state(0) == 1 && midi->get_value() >= 3) {
      params->blog(
	LOG_INFO, "<Audio Sources> action failed - MIDI value exceeds audio monitor option count.");
      return;
    }
    obs_source_set_monitoring_type(obs_source,
...
template<>
void MMGAction::do_action_specific<MMGAction::MediaSources>(const MMGAction *params,
							    const MMGMessage *midi)
{
  OBSSourceAutoRelease obs_source = obs_get_source_by_name(params->get_str(0).qtocs());
  if (!obs_source) {
    params->blog(LOG_INFO, "<Media Sources> action failed - Source does not exist.");
    return;
  }
  if (!(obs_source_get_output_flags(obs_source) & OBS_SOURCE_CONTROLLABLE_MEDIA)) {
    params->blog(LOG_INFO, "<Media Sources> action failed - Source is not a media source.");
    return;
  }
  ...
  case MMGAction::MediaSources::SOURCE_MEDIA_TIME:
    obs_source_media_set_time(
      obs_source,
      num_or_value(params, midi, 0, 0, get_obs_media_length(params->get_str(0))) * 1000);
    break;
  ...
  case MMGAction::MediaSources::SOURCE_MEDIA_SKIP_FORWARD_TIME:
    obs_source_media_set_time(obs_source,
			      obs_source_media_get_time(obs_source) + params->get_num(0));
    break;
...
template<>
void MMGAction::do_action_specific<MMGAction::Transitions>(const MMGAction *params,
							   const MMGMessage *midi)
{
  OBSSourceAutoRelease obs_transition = obs_get_transition_by_name(params->get_str(0).qtocs());
  if (!obs_transition)
    obs_transition = obs_frontend_get_current_transition();
  int obs_transition_time =
    num_or_value(params, midi, 0, obs_frontend_get_transition_duration(), 3200);
  OBSSourceAutoRelease obs_source = obs_get_source_by_name(params->get_str(2).qtocs());
  OBSSceneAutoRelease obs_scene = obs_get_scene_by_name(params->get_str(1).qtocs());
  ...
  case MMGAction::Transitions::TRANSITION_SOURCE_SHOW:
    if (!obs_sceneitem) {
      params->blog(LOG_INFO, "<Transitions> action failed - Source in scene does not exist.");
      return;
    }
    obs_sceneitem_set_transition(obs_sceneitem, true, obs_transition);
    obs_sceneitem_set_transition_duration(obs_sceneitem, true, obs_transition_time);
    break;
  case MMGAction::Transitions::TRANSITION_SOURCE_HIDE:
    ...
    obs_sceneitem_set_transition_duration(obs_sceneitem, false, obs_transition_time);
    break;
  default:
    break;
  }
						       ...
						       const MMGMessage *midi)
{
  ...
  OBSSourceAutoRelease obs_filter =
    obs_source_get_filter_by_name(obs_source, params->get_str(1).qtocs());
  if (!obs_filter) {
    params->blog(LOG_INFO, "<Filters> action failed - Filter in source does not exist.");
    return;
  }
  //OBSDataAutoRelease filter_data = obs_source_get_settings(obs_filter);
  //QJsonDocument doc = QJsonDocument::fromJson(obs_data_get_json(filter_data));
  ...
  case MMGAction::Filters::FILTER_REORDER:
    if (num_or_value(params, midi, 0, 0, 128.0, 1) - 1 >= obs_source_filter_count(obs_source)) {
      params->blog(LOG_INFO,
    ...
    for (int i = 0; i < num_or_value(params, midi, 0, 0, 128.0, 1) - 1; ++i) {
						       ...
						       const MMGMessage *midi)
{
  ...
  obs_enum_hotkeys(
    [](void *data, obs_hotkey_id id, obs_hotkey_t *hotkey) {
      ...
      const char *current_hotkey_name = obs_hotkey_get_description(hotkey);
  ...
  if (req.hotkey_id == (size_t)(-1)) {
    params->blog(LOG_INFO, "<Hotkeys> action failed - Hotkey does not exist.");
    return;
  }
							...
							const MMGMessage *midi)
{
  ...
  if (params->get_str(0) == "Use Message Value" &&
    ...
    params->blog(LOG_INFO, "<Profiles> action failed - MIDI value exceeds profile count.");
    return;
  }
  ...
  if (params->get_sub() == 0) {
    if (!(obs_frontend_streaming_active() || obs_frontend_recording_active() ||
      ...
      set_profile(params->get_str(0) == "Use Message Value" ? profile_names[midi->get_value()]
							    : params->get_str(0).qtocs());
    }
  }
...
template<>
void MMGAction::do_action_specific<MMGAction::Collections>(const MMGAction *params,
							   const MMGMessage *midi)
{
  char **collection_names = obs_frontend_get_scene_collections();
  ...
  if (params->get_sub() == 0) {
    set_collection(params->get_str(0) == "Use Message Value" ? collection_names[midi->get_value()]
							     : params->get_str(0).qtocs());
  }
...
template<>
void MMGAction::do_action_specific<MMGAction::MidiMessage>(const MMGAction *params,
							   const MMGMessage *midi)
{
  libremidi::message message;
  if (params->get_str(1) == "Note On") {
    message = libremidi::message::note_on(params->get_num(0), num_or_value(params, midi, 1),
					  num_or_value(params, midi, 2));
  } else if (params->get_str(1) == "Note Off") {
    message = libremidi::message::note_off(params->get_num(0), num_or_value(params, midi, 1),
					   num_or_value(params, midi, 2));
  } else if (params->get_str(1) == "Control Change") {
    message = libremidi::message::control_change(params->get_num(0), num_or_value(params, midi, 1),
						 num_or_value(params, midi, 2));
  } else if (params->get_str(1) == "Program Change") {
    message = libremidi::message::program_change(params->get_num(0), num_or_value(params, midi, 1));
  } else if (params->get_str(1) == "Pitch Bend") {
    int num1 = num_or_value(params, midi, 1);
    message =
      libremidi::message::pitch_bend(params->get_num(0), num1 >= 64 ? ((num1 - 64) << 1) : 0, num1);
  } else {
    params->blog(LOG_INFO, "<MIDI> action failed - Set MIDI message is invalid.");
    return;
  }
  if (params->get_sub() == 0) {
    MMGDevice *const output = global()->find_device(params->get_str(0));
    if (!output) {
      params->blog(LOG_INFO,
							...
							const MMGMessage *midi)
{
  ...
  MMGSharedMessage original = *reinterpret_cast<MMGSharedMessage *>((qulonglong)params->get_num(0));
  if (original == 0) {
    open_message_box("Error",
						       ...
						       const MMGMessage *midi)
{
  ...
  case MMGAction::Timeout::TIMEOUT_MS:
    std::this_thread::sleep_for(std::chrono::milliseconds((int)num_or_value(params, midi, 0)));
    break;
  case MMGAction::Timeout::TIMEOUT_S:
    std::this_thread::sleep_for(std::chrono::seconds((int)num_or_value(params, midi, 0)));
    break;
  default:
    break;
  }
